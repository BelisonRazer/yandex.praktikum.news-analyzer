...
export class CardCommitList {
  ...
  addCard(cName, cEmail, cDate, cMessage, cImageUrl) {
    const { cardElement } = new CardCommit(
      cName,
      cEmail,
      cDate,
      cMessage,
  ...
  render() {
    for (let i = 0; i < this.serverCard.length; i++) {
      ...
      this.addCard(
        this.serverCard[i].commit.committer.name,
        this.serverCard[i].commit.committer.email,
        resultDate,
        this.serverCard[i].commit.message,
        this.serverCard[i].author.avatar_url
      );
    }
