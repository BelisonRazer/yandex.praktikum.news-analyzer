...
export default class Statistics {
  ...
  viewStatistics(count) {
    ...
    document.querySelector(
      ".answer__weekly-count"
    ).textContent = this.data.totalResults;
  ...
  toGraph(artPerDay) {
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      if (day in artPerDay) {
        ...
        const widthOfPercent =
          (artPerDay[`${day}`] * 100) / this.data.totalResults + 2.2;

        document.querySelector(
          `.graph-${i}`
        ).style.width = `${widthOfPercent}%`;
        document.querySelector(`.numb-${i}`).textContent = `${
          artPerDay[`${day}`]
        }`;
      } else {
